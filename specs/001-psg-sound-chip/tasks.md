...
- [X] T055 Final `make coverage`: 100% line, 100% FSM state/transition, 100% toggle on registers and ports; every hole closed with a test, removed as dead logic, or waived with a written reason (Principle II) **Closed 2026-09-14** (Verilator 5.040, full 11-module 51-test suite, 51/51 pass, 427/445 = 95.00%): line 110 pts / 3 zero, branch 102 / 0, expression 107 / 0, toggle 1752 / 52 zero. Every hole dispositioned — 3 line + 48 toggle **waived with written proofs** in [plan.md](plan.md) Complexity Tracking (two rows); the remaining 4 toggle points (`ui_in[7]`) **closed with a test**, `test_reset.py::test_ui_in7_unused`, rather than waived. FSM state/transition coverage is 100% via cocotb-coverage (`bus.fsm.state` 4/4, `bus.fsm.transition` 9/9, `bus.cycle.cross` 8/8) with **zero unhit bins anywhere in the suite**.
- [X] T056 Complete `traceability.md`: every FR-001…FR-060 and SC-001…SC-009 maps to a passing test (Principle II) **Closed 2026-09-14.** All 41 rows dispositioned: **36 passing**, 4 gate-deferred (FR-051, FR-060, SC-008, SC-009 — area, GL sim, first silicon, STA), 1 by construction (FR-050, an environmental constraint declared in `info.yaml` and enforced by the TT flow, deliberately **not** cited to `test_timebase.py` since that bench's `CLK_PERIOD_PS` is a stimulus constant and the citation would be circular). Zero unmapped, zero partial. Four process findings recorded in `traceability.md` — a suite that ran nowhere while CI stayed green, relative-path coverage-artifact collisions under concurrency, `make clean` not removing coverage YAMLs, and a mislabeled coverage bin that made 88.89% wrong in both directions.
...
- [X] T061 Walk through `quickstart.md` end to end and correct any drift **Done 2026-09-14.** Corrections: (1) §2's module table listed 5 of 12 suites — now all eleven cocotb modules plus a separate table for the one pytest file, with the reason `make` cannot run it; (2) "a bare `make -B` now runs every suite" was false once the suite split in two — now states eleven cocotb suites, ~40 min, and documents `make unit` / `make all-tests`; (3) §3 was headed "Area check" while also running lint/fpga/coverage — retitled, and its R11 threshold bullets (which still told the reader to consider cutting S/PDIF) marked **superseded** by T049's decision three lines below rather than deleted, so the reasoning stays legible without the stale directive; (4) §1 claimed six things the IP bench does not test — the randomized ≥10,000-cycle readback, unselected cycles and back-to-back 60 ns gap are chip-level checks in `test_bus.py`; replaced with the bench's actual 8 tests mapped to contract guarantees 1–7 plus the parameterization smoke test; (5) prerequisites now record the venv failure mode (system `python3` may carry cocotb without `cocotb-coverage`, dying at `import cocotb_coverage` in `test_bus.py`). Documented commands verified by running them: `make unit` 4/4, `bus68k_if` bench 8/8.
